...
describe('OrderStateMachineService.validateTransition', () => {
  it('allows the canonical forward path', () => {
    expect(() =>
      OrderStateMachineService.validateTransition('pending', 'accepted')
    ).not.toThrow();
    expect(() =>
      OrderStateMachineService.validateTransition('accepted', 'preparing')
    ).not.toThrow();
    expect(() =>
      OrderStateMachineService.validateTransition('preparing', 'ready')
    ).not.toThrow();
  ...
  it('allows cancellation from pending and accepted', () => {
    expect(() =>
      OrderStateMachineService.validateTransition('pending', 'canceled')
    ).not.toThrow();
    expect(() =>
      OrderStateMachineService.validateTransition('accepted', 'canceled')
    ).not.toThrow();
  });

  it('rejects illegal transitions', () => {
    expect(() =>
      OrderStateMachineService.validateTransition('completed', 'pending')
    ).toThrow(AppError);
    expect(() =>
      OrderStateMachineService.validateTransition('canceled', 'preparing')
    ).toThrow(AppError);
    expect(() =>
      OrderStateMachineService.validateTransition('completed', 'accepted')
    ).toThrow(AppError);
  });
