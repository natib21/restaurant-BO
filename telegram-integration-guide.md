...
### 4.6 `POST /api/v1/customer/:customerId/telegram/link-token` — _planned, not yet implemented_
...
- The webhook endpoint is intentionally unauthenticated (Telegram cannot send a Bearer token) — the secret header check _is_ the authentication for that route.
